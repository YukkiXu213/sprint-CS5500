...
async def get_clients_by_criteria(
    ...
    return ClientFilterService.get_clients_by_criteria(
        db,
...
async def get_clients_by_services(
    ...
    return ClientCaseService.get_clients_by_services(
        db,
...
async def get_clients_by_success_rate(
    min_rate: int = Query(70, ge=0, le=100),
    current_user: User = Depends(get_admin_user),
    db: Session = Depends(get_db)
):
    return ClientCaseService.get_clients_by_success_rate(db, min_rate)
